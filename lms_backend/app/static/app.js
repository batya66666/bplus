...
async function loadMentorReports() {
  ...
  // показываем блок только ментору/тимлиду/админу
  ...
  renderList(box, list, (r) => {
    ...
    acceptBtn.addEventListener("click", async () => {
      try {
        ...
        await loadMyReports(); // чтобы сотрудник видел обновления при смене аккаунтов/тестах
...
// ---------- ADMIN: departments/users/assign ----------
...
document.querySelectorAll(".tab").forEach(b => {
  b.addEventListener("click", async () => {
    ...
    try {
      if (name === "courses") await loadMyCourses();
      if (name === "standups") {
        await loadMyReports();
        await loadMentorReports(); // внутри сама проверит роль и скроет блок если не надо
      }
