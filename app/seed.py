...
PUBLIC_COURSES = [
    dict(title="Онбординг", description="Базовый курс для всех", is_mandatory=True, deadline_days=7, is_public=True),
    dict(title="Инфобез", description="Правила безопасности", is_mandatory=True, deadline_days=3, is_public=True),
]

async def seed_public_courses(session):
    for c in PUBLIC_COURSES:
        exists = (await session.execute(select(Course).where(Course.title == c["title"]))).scalar_one_or_none()
        if not exists:
